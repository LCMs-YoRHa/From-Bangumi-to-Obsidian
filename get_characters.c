...
void get_characters(const int *collection_id)
{
    ...
    {                                              // 遍历数组的每个元素
        ...
        if (cJSON_IsString(cname))
            strncpy(ch->name, cname->valuestring, sizeof(ch->name)); // 如果是字符串类型，则复制到ch->name中

        // 角色关系
        cJSON *crel = cJSON_GetObjectItem(item, "relation");
        if (cJSON_IsString(crel))
            strncpy(ch->relation, crel->valuestring, sizeof(ch->relation)); // 同理
        ...
        {                                                      //
            ...
            if (cJSON_IsString(grid))
                strncpy(ch->char_image, grid->valuestring, sizeof(ch->char_image)); // 同理
        }
        ...
        { // 确保actors是数组类型且长度大于0
            ...
            if (actor)
            {
                ...
                if (cJSON_IsString(aname))
                    strncpy(ch->actors[0].name, aname->valuestring, sizeof(ch->actors[0].name)); // 同理
                ...
                if (cJSON_IsObject(aimg))
                {
                    ...
                    if (cJSON_IsString(agrid))
                        strncpy(ch->actors[0].grid, agrid->valuestring, sizeof(ch->actors[0].grid)); // 同理
                }

...
void readcerts()
{
    ...
    // 读取文件里的内容，检查格式是否正确
    if (fscanf(file, "username=%50s\ntoken=%100s", username, token) != 2) // 从文件读两行，第一行是username=后面接用户名，第二行是token=后面接令牌
    ...
    snprintf(auth_header, sizeof(auth_header), "Authorization: Bearer %s", token); // 拼出身份认证头，比如"Authorization: Bearer 用户的token"
    snprintf(user_agent, sizeof(user_agent), "User-Agent: %s/WORK", username);     // 拼出用户代理头，比如"User-Agent: 用户用户名/WORK"
}